...
describe('JsonRpcTransport', () => {
  ...
  describe('sendRequest and response correlation', () => {
    ...
    it('keeps client requests pending when server requests reuse their ids', async () => {
      const promise = transport.sendRequest('workspace/executeCommand', {});
      const written = mock.stdinData[0] as string;
      ...
      transport.sendMessage({ jsonrpc: '2.0', id: sent.id, result: [] });
      mock.simulateResponse({ jsonrpc: '2.0', id: sent.id, result: { body: [] } });
      expect(await promise).toEqual({ body: [] });
    });

    it('ignores id-only messages until a real null result arrives', async () => {
      const promise = transport.sendRequest('textDocument/hover', {});
      const written = mock.stdinData[0] as string;
      const sent = JSON.parse(written.substring(written.indexOf('{'))) as LSPMessage;
      mock.simulateResponse({ jsonrpc: '2.0', id: sent.id });
      mock.simulateResponse({ jsonrpc: '2.0', id: sent.id, result: null });
  ...
  describe('over-bound response bodies', () => {
    ...
    it('correlates each oversized response to its own concurrent request', async () => {
      ...
      for (const id of [2, 1]) {
        ...
        mock.stdout.emit(
          'data',
          Buffer.from(`Content-Length: ${Buffer.byteLength(content)}\r\n\r\n${content}`, 'utf8')
        );
      }
      ...
      const files = [firstError, secondError].map(
        (error) => (error as LspToolOutcomeError).outcome.resultFile
      );
      expect(new Set(files).size).toBe(2);
      expect((firstError as LspToolOutcomeError).outcome.bytes).toBe(Buffer.byteLength(payload(1)));
      expect((secondError as LspToolOutcomeError).outcome.bytes).toBe(
        Buffer.byteLength(payload(2))
      );
    });
      ...
      for (const frame of [
        {
          jsonrpc: '2.0',
          method: 'window/logMessage',
          params: { message: 'z'.repeat(4096), id: 1 },
        },
        {
          jsonrpc: '2.0',
          id: 1,
          method: 'workspace/applyEdit',
          params: { edit: 'y'.repeat(4096) },
        },
      ]) {
    ...
    it('resynchronizes on the next frame after an over-bound body', async () => {
      ...
      mock.stdout.emit(
        'data',
        Buffer.from(`Content-Length: ${Buffer.byteLength(big)}\r\n\r\n${big}`, 'utf8')
      );
      await pending.then(
        () => null,
        () => null
